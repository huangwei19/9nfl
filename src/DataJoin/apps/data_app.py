...
def query_data_block_meta():
    ...
    if not data_block_metas:
        return get_result(retcode=101, retmsg='find data block meta failed')
    return get_result(retcode=0, retmsg='success',
...
def query_data_source_meta():
    ...
    if not data_source_metas:
        return get_result(retcode=101, retmsg='find data source meta failed')
    return get_result(retcode=0, retmsg='success',
...
def query_data_source():
    ...
    if not data_sources:
        return get_result(retcode=101, retmsg='find data source failed')
    return get_result(retcode=0, retmsg='success', data=[data_source.to_json() for data_source in data_sources])
