...
class FlBridgeServerInitOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES(ctx, input_tensor->NumElements() > 0, errors::InvalidArgument("server_address size should > 0"));
    
    Params conf;
    
    OP_REQUIRES_OK(ctx, ctx->input("appli_id", &input_tensor));
    OP_REQUIRES(ctx, input_tensor->NumElements() > 0, errors::InvalidArgument("appli_id size should > 0"));
    auto appli = input_tensor->flat<string>();
    conf.appli_id = appli(0);
    OP_REQUIRES_OK(ctx, ctx->input("rank_id", &input_tensor));
    OP_REQUIRES(ctx, input_tensor->NumElements() > 0, errors::InvalidArgument("rank_id size should > 0"));
    auto rand = input_tensor->flat<int>();
    conf.rank_id = rand(0);
    OP_REQUIRES_OK(ctx, ctx->input("role_def", &input_tensor));
    OP_REQUIRES(ctx, input_tensor->NumElements() > 0, errors::InvalidArgument("role_def size should > 0"));
    auto role = input_tensor->flat<string>();
    conf.role_def = role(0);
    OP_REQUIRES_OK(ctx, ctx->input("rpc_service_type", &input_tensor));
    OP_REQUIRES(ctx, input_tensor->NumElements() > 0, errors::InvalidArgument("rpc_service_type size should > 0"));
    auto rpc_service_type = input_tensor->flat<int>();
    if (rpc_service_type(0) == 0) {
      conf.service_type = KindOfServiceType::kUnary;
      LOG(INFO) << "KindOfServiceType: " << rpc_service_type(0) << "(Unary RPCs)";
    }
    else {
      conf.service_type = KindOfServiceType::kBidiStreaming;
      LOG(INFO) << "KindOfServiceType: " << rpc_service_type(0) << "(BidiStreaming RPCs)";
    }
    ...
    for (int i = 0; i < meta_array.size(); ++i) {
      LOG(INFO) << "Add Ctx Metadata, " << "uuid" << " : " << meta_array(i);
      conf.ctx_meta_conf.push_back(std::make_pair("uuid", meta_array(i)));
    }
    LOG(INFO) << "FlBridge Params:" \
      << "appli_id(" << conf.appli_id << "), rank_id(" << conf.rank_id << "), role_def(" << conf.role_def << ")";
...
REGISTER_KERNEL_BUILDER(Name("FlBridgeServerInit").Device(DEVICE_CPU), FlBridgeServerInitOp);

REGISTER_KERNEL_BUILDER(Name("FlRpcChannelInit").Device(DEVICE_CPU), FlRpcChannelInitOp);
