...
void DumpMsg(TrainerWorkerMessage& msg) {
 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    for (int i = 0; i < retries; i++) {
      s = bridge_api->QueryResult(0, FL_Key_DataMessage, datamsg_rname_, &msg_recv);
      if (s.ok()) { 
        if (FlDebugging() > 99) {
          DumpMsg(msg_recv);
        }
...
class FlTensorSendOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if (FlDebugging() > 99) {
      LOG(INFO) << "Send tensor: " << input_tensor->DebugString(max_length_dbgmsg);
    }
    ...
    msg_send.mutable_data()->set_iter_id(bridge_mgr_->StepStats()->CurrentIterId());
    ...
    for (int i = 0; i < retries; i++) {
      ...
      if (s.ok()) {
        ...
        LOG(ERROR) << "FlTensorSendOp data transmit failed: " << s.error_message();
...
class FlTrainStartOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    int64_t current_seq = bridge_mgr_->StepStats()->CurrentSeqNum();
    msg_send.set_seq_num(current_seq);
    msg_send.mutable_start()->set_iter_id(bridge_mgr_->StepStats()->CurrentIterId());
    ...
    TrainerWorkerMessage msg_recv;
    for (int i = 0; i < retries; i++) {
      s = bridge_api->QueryResult(0, FL_Key_StepCommit, FL_Key_StepCommit, &msg_recv);
      if (s.ok()) { 
        if (FlDebugging() > 99) {
          DumpMsg(msg_recv);
        }
...
class FlTrainFollowOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    int retries = RetryMaxCorrected(max_retries_);
    for (int i = 0; i < retries; i++) {
      s = bridge_api->QueryResult(0, FL_Key_TrainStart, FL_Key_TrainStart, &msg_recv);
      if (s.ok()) { 
        if (FlDebugging() > 99) {
          DumpMsg(msg_recv);
        }
    ...
    msg_send.mutable_commit()->set_round_id(bridge_mgr_->StepStats()->CurrentIterId());
    TrainerWorkerResponse resp;
 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    for (int i = 0; i < retries; i++) {
      ...
      LOG(ERROR) << "FlTensorSendRecvOp data transmit failed: " << s.error_message();
      if ( (i + 1) == retries ) { break; }
      LOG(INFO) << "Sleeping for: " << sleep_in_ms;
      ctx->env()->SleepForMicroseconds(sleep_in_ms);
    }
...
class FlTrainStepCommitOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    int64_t current_seq = bridge_mgr_->StepStats()->CurrentSeqNum();
    msg_send.set_seq_num(current_seq);
    msg_send.mutable_commit()->set_iter_id(bridge_mgr_->StepStats()->CurrentIterId());
    TrainerWorkerResponse resp;
...
#define REGISTER_KERNEL(type)                               \
  REGISTER_KERNEL_BUILDER(                                  \
      Name("FlGradBackpropRequest").Device(DEVICE_CPU),     \
      FlTensorSendOp<type>)
...
REGISTER_KERNEL_BUILDER(Name("FlTensorRecvWithGradBp").Device(DEVICE_CPU), 
          FlTensorRecvOp);
REGISTER_KERNEL_BUILDER(Name("FlGradRecv").Device(DEVICE_CPU), 
          FlTensorRecvOp);
